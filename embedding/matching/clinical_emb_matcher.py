...
from utils_functions.string_manager import clinical_embedding_vector_db_path, clinical_embedding_model_path


class ClinicalEmbMatcher:

    def __init__(self, text_processor: TextPreprocessing, n_result: int = 500):
        vectors_storage_path: str = clinical_embedding_vector_db_path
        self.vector_db_instance = VectorDBHelper.get_instance(db_path=vectors_storage_path)
        self.text_processor = text_processor
        self.model_name: str = 'clinical'
        self.n_result = n_result

        model_storage_path: str = clinical_embedding_model_path
